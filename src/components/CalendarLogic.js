...
const CalendarLogic = () => {
  const [title, setTitle] = useState(""), [date, setDate] = useState(""), [time, setTime] = useState(""),
    [events, setEvents] = useState(loadEvents), [search, setSearch] = useState(""),
    [filter, setFilter] = useState("all"), [viewDate, setViewDate] = useState(""),
    [visible, setVisible] = useState([]), [sys, setSys] = useState([]),
    [editId, setEditId] = useState(null), [editTitle, setEditTitle] = useState(""),
    [editDate, setEditDate] = useState(""), [editTime, setEditTime] = useState("");
  ...
  const persistEvents = () => saveEvents(events);
  const startTimerTick = () => {
    const tick = () => {
      const t = Date.now();
      events.forEach((e) => {
        if (new Date(e.datetime).getTime() <= t && !visible.includes(e.id) && !fired.current.has(e.id)) {
  ...
  const map = useMemo(() => {
    ...
    for (const e of events) {
      const k = ymd(new Date(e.datetime));
      if (!m.has(k)) m.set(k, []);
      m.get(k).push(e);
    }
    return m;
  ...
  return (
    <CalendarView
      {...{ title, setTitle, date, setDate, time, setTime, addEvent, filtered,
        editId, setEditId, editTitle, setEditTitle, editDate, setEditDate,
        editTime, setEditTime, saveEdit, del, startEdit, filter, setFilter,
        viewDate, setViewDate, search, setSearch, visible, sys, closeToast,
        closeSys, year, setYear, prevYear, nextYear, mode, month, openMonth,
        backToYear, prevMonth, nextMonth, map, onDayClick }}
    />
  );
};
