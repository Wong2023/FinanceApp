...
import { useLanguage } from "./LanguageContext"; // ✅ добавил перевод

// Ссылка на иконку с Imgur
const iconUrl = "https://i.imgur.com/vD4fJbl.png"; // Прямой URL на изображение

export default function Sidebar() {
  const [hovered, setHovered] = useState("");
  const navigate = useNavigate();
  const location = useLocation();
  const { t } = useLanguage(); // ✅ подключаем переводчик
  ...
  return (
    <SidebarContainer bg={sidebarColor}>
      ...
      {/* 🔻 Новый независимый нижний блок — Calendar */}
      <MenuBlock>
  <Title
